...
procedure TAnalyzerForm.Execute(DataSet: TDataSet);
begin
...
begin
  if DataSet.AttributesCount < 2
    then raise Exception.Create('Imposible analysis: attributes missing')
    else
      begin
        Result := TIntSet.Create;
        for i := 0 to DataSet.AttributesCount - 2 do
...
begin
  ...
  if fDiscernability.Cardinality = 0
    then raise EMetric.Create('It is not possible to apply metric: "Mean class rough involvement"')
    else
      begin
...
begin
  Result := 0;
  if fDiscernability.Cardinality = 0
    then raise EMetric.Create('It is not possible to apply metric:  "Mean class rough membership"')
    else
      begin
        for j := 0 to pred(fDiscernability.Cardinality) do
          begin
...
procedure TAnalyzerForm.InformationTheoryAnalysis(DataSet: TDataSet);
begin
  AnalysisThread := TAnalysisThread.Create(Self);
  AnalysisThread.ExecuteAnalysis(DataSet, atInformationTheory);
end;

procedure TAnalyzerForm.PrintMatrix(DataSet: TDataSet; const aTitle: string; aMatrix: T2DMatrixSingle);
var
  i, j: integer;
  S: string;
begin
  ...
  for i := 0 to pred(DataSet.AttributesCount) do
    begin
      S := DataSet.Attribute[i].Name + chr(9) + chr(9) + chr(9);
...
procedure TAnalyzerForm.RoughSetAnalysis(DataSet: TDataSet);
begin
  AnalysisThread := TAnalysisThread.Create(Self);
...
begin
  if (Value < 0) or (Value >= DataSet.ClassCardinality)
    then raise Exception.Create(Format('Invalid class index "%d"', [Value]))
...
begin
  if DataSet.InstanceNumber = 0
    then raise Exception.Create('Imposible analysis: no instances in data set')
    else
      begin
        Result := TIntSet.Create;
        for i := 0 to pred(DataSet.InstanceNumber) do
...
procedure TAnalyzerForm.FormHide(Sender: TObject);
begin
  Main.DataSetDesigner.ViewAnalysisSheet.Checked := false;
end;

procedure TAnalyzerForm.FormDeactivate(Sender: TObject);
begin
  Main.DataSetDesigner.ViewAnalysisSheet.Checked := false;
end;
...
procedure TAnalyzerForm.BasicMeasures(DataSet: TDataSet);
begin
  AnalysisThread := TAnalysisThread.Create(Self);
...
begin
  if N[Ci] < 2
    then raise EMetric.Create('It is not possible to apply metric SD_Ratio: there are classes that have less than two instances')
    else K := 1 / (N[Ci] - 1);
...
procedure TAnalyzerForm.StatisticAnalysis(DataSet: TDataSet);
begin
  AnalysisThread := TAnalysisThread.Create(Self);
...
begin
  with F do
    begin
      ...
      Memo.Lines.Add('');
      Memo.Lines.Add('   BASIC METRICS');
      Memo.Lines.Add('');
      Memo.Lines.Add('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO');
      Memo.Lines.Add('');
      Memo.Lines.Add('');
      Memo.Lines.Add(Format('Number of Instances: %d', [DataSet.InstanceNumber]));
      Memo.Lines.Add('');
      Memo.Lines.Add(Format('Number of Decision Attributes: %d', [pred(DataSet.AttributesCount)]));
      Memo.Lines.Add('');
      Memo.Lines.Add(Format('Number of Classes: %d', [DataSet.ClassCardinality]));
      Memo.Lines.Add('');
      Memo.Lines.Add(Format('Number of Binary Decision Attributes: %d', [DataSet.BinAttrNumber]));
      ProgressBar1.Position := 1;
    end;
end;

constructor TAnalysisThread.Create(Form: TAnalyzerForm);
begin
...
begin
  ...
  if Stop then
    begin
      F.Memo.Lines.Add('');
      F.Memo.Lines.Add('The process have been canceled by the user');
      F.ProgressBar1.Position := 0;
    end;
...
begin
  with F do
    begin
      ...
      Memo.Lines.Add('');
      Memo.Lines.Add('   INFORMATION THEORY METRICS ');
      Memo.Lines.Add('');
      Memo.Lines.Add('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO');
      Memo.Lines.Add('');
      Memo.Lines.Add('');
      ...
      try
        ...
        ITA.Analyze;
        if Stop then exit;
        Memo.Lines.Add('*** Prior Probability Matrix ***');
        Memo.Lines.Add('');
        for i := 0 to pred(DataSet.AttributesCount) do
          begin
        ...
        PrintMatrix(DataSet, 'Joint Entropy Matrix ', ITA.fJointEntropyMatrix);
        PrintMatrix(DataSet, 'Mutual Information Matrix ', ITA.fMutualInfoMatrix);
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Class Entropy ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('H(C) = %.4F', [ITA.ClassEntropy]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Mean Entropy ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('H(X) = %.4F', [ITA.AttrMeanEntropy]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Average Mutual Information ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('M(C, X) = %.4F', [ITA.AverageMutualInformation]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Number of Equivalent Attributes ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('EN.Attr = %.4F', [ITA.ENAttr]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Noise Ratio ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('Noise Ratio = %.4F', [ITA.Noisiness]));
      except
        on E: Exception do
...
begin
  ...
  with F do
    begin
      ...
      Memo.Lines.Add('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO');
      Memo.Lines.Add('');
      Memo.Lines.Add('   ROUGH SET THEORY METRICS ');
      Memo.Lines.Add('');
      Memo.Lines.Add('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO');
      Memo.Lines.Add('');
      ...
      try
        ...
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Equivalent Relations ***');
        Memo.Lines.Add('');
        ...
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Attributes Discernibility ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('Dis = %S', [RSA.PrintDiscernability]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Lower Approximation Vector ***');
        Memo.Lines.Add('');
        Memo.Lines.Add(RSA.PrintLowerApprox);
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Upper Approximation Vector ***');
        Memo.Lines.Add('');
        ...
        RSA.ComputMeasures;
        if Stop then exit;
        Memo.Lines.Add('*** Class Approximation Precision Vector ( |R-| / |R+| ) ***');
        Memo.Lines.Add(RSA.PrintVector(RSA.fApproxAccuracy));
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('Mean Classification Accuracy = %f', [RSA.ClassifAccuracy]));
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('Weighted Classification Accuracy = %f', [RSA.WeightedClassifAccuracy]));
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('General Classification Accuracy = %f', [RSA.GenClassifAccuracy]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Mean Class Approximation Membership (General Accuracy Weights) ***');
        Memo.Lines.Add(RSA.PrintVector(RSA.fWeights1));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Class Approximation Quality Vector ( |R-| / |X| ) ***');
        Memo.Lines.Add(RSA.PrintVector(RSA.fApproxQuality));
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('Mean Classification Quality = %f', [RSA.ClassifQuality]));
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('General Classification Quality = %f', [RSA.GenClassifQuality]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Mean Class Approximation Agreement (General Accuracy Weights) ***');
        Memo.Lines.Add(RSA.PrintVector(RSA.fWeights2));
        Memo.Lines.Add('');
        Memo.Lines.Add(Format('General Approximation Coefficients = %f', [RSA.GenApproxRatio]));
        Memo.Lines.Add('');
        Memo.Lines.Add('*** Class Dependence Degree Vector ( |R-| / |U| ) ***');
...
begin
  ...
  with F do
    begin
      ...
      Memo.Lines.Add('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO');
      Memo.Lines.Add('');
      Memo.Lines.Add('   STATISTIC THEORY METRICS ');
      Memo.Lines.Add('');
      Memo.Lines.Add('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO');
      Memo.Lines.Add('');
      Memo.Lines.Add('');
...
procedure TAnalyzerForm.StopBtnClick(Sender: TObject);
begin
