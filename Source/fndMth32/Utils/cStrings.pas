...
function  StrAfterChar(const S: AnsiString; const D: CharSet): AnsiString; overload;
function  StrAfterChar(const S: AnsiString; const D: Char): AnsiString; overload;
...
begin
  ...
  if not Result or (L = 0) then
    exit;
  Result := StrPCompareNoCase(Pointer(A), Pointer(B), L) = crEqual;
end;
...
begin
  ...
  if not Result or (L1 = 0) then
    exit;
  if CaseSensitive then
    Result := StrPCompare(Pointer(A), Pointer(B), L1) = crEqual else
...
begin
  ...
  if not Result or (Len1 = 0) then
    exit;
  if CaseSensitive then
    Result := StrPCompare(P1, P2, Len1) = crEqual else
...
begin
  ...
  if not Result or (Len = 0) then
    exit;
  if CaseSensitive then
    Result := StrPCompare(P, Pointer(S), Len) = crEqual else
...
begin
  ...
  Dec(L, M - 1);
  While I <= L do
    if StrPMatchStr(P, F, CaseSensitive) then
      begin
        Result := I;
        exit;
      end else
...
var L, M : Integer;
    P : PChar;
begin
  if (Prefix <> '') and not StrMatchLeft(Prefix, S, CaseSensitive) then
    begin
      L := Length(S);
      M := Length(Prefix);
      SetLength(S, L + M);
...
var L, M : Integer;
    P : PChar;
begin
  if (Suffix <> '') and not StrMatchRight(Suffix, S, CaseSensitive) then
    begin
      L := Length(S);
      M := Length(Suffix);
      SetLength(S, L + M);
...
begin
  ...
  if not Assigned(Q) then
    exit;
  While (Q^ <> #0) and not StrPMatchStr(Q, S, CaseSensitive) do
    begin
      Inc(Q);
      Inc(Result);
    end;
...
begin
  Q := P;
  L := 0;
  While (P^ <> #0) and not StrPMatchStr(P, S, CaseSensitive) do
    begin
      Inc(P);
      Inc(L);
    end;
  ...
  begin
    Case Ch of
      ...
      asciiVT   : Result := '\v';
        ...
        Result := '\x' + IntToHex(Ord(Ch), 1) else
        Result := Ch;
    end;
  end;
...
begin
  ...
  if L = 1 then
    if S[1] in ['.', '*', '?'] then
      ...
      Result := [S[1]] else
  if (S[1] <> '[') or (S[L] <> ']') then
    raise EConvertError.Create('Invalid character class string') else
    begin
      Neg := S[2] = '^';
      I := iif(Neg, 3, 2);
      Result := [];
      ...
      if MaxI(PrevPos + 1, 0) = 0 then // find first
        begin
          ...
          Repeat
            ...
            if StrCompareNoCase(V[I], Find) = crGreater then
              H := I - 1 else
              L := I + 1;
          Until L > H;
          Result := -1;
...
begin
  ...
  Assert(not StrMatch('xx', 'abcdef', 1), 'StrMatch');
  Assert(StrMatch('x', 'xbcdef', 1), 'StrMatch');
  Assert(StrMatch('xxxxx', 'abcdxxxxx', 5), 'StrMatch');
  Assert(StrMatch('abcdef', 'abcdef', 1), 'StrMatch');
  Assert(not StrMatchNoCase('xx', 'abcdef', 1), 'StrMatchNoCase');
  Assert(StrMatchNoCase('xBCd', 'xbCDef', 1), 'StrMatchNoCase');
  Assert(StrMatchNoCase('Xxx-xX', 'abcdxxX-xx', 5), 'StrMatchNoCase');
  Assert(StrMatch('abcd', 'abcde', 1), 'StrMatch');
  Assert(StrMatch('abc', 'abcde', 1), 'StrMatch');
  Assert(StrMatch('ab', 'abcde', 1), 'StrMatch');
  Assert(StrMatch('a', 'abcde', 1), 'StrMatch');
  Assert(StrMatchNoCase(' abC-Def{', ' AbC-def{', 1), 'StrMatchNoCase');
  Assert(StrMatchLeft('aBc1', 'ABC1D', False), 'StrMatchLeft');
  Assert(StrMatchLeft('aBc1', 'aBc1D', True), 'StrMatchLeft');
  Assert(not StrMatchLeft('ABc1', 'AB1D', False), 'StrMatchLeft');
  Assert(not StrMatchLeft('aBc1', 'aBC1D', True), 'StrMatchLeft');
